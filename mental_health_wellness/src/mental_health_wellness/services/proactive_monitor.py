"""
Proactive Mental Health Monitor — Background Service
...
def _detect_alerts(mood_logs: list) -> list[str]:
    ...
    if len(intensities) >= 7:
        drift = intensities[0] - intensities[6]
        if drift > 0.15:
            alerts.append("gradual_mood_decline")

    recent_5 = intensities[:5]
    if sum(1 for i in recent_5 if i >= 0.65) >= 3:
    ...
    if sum(1 for i in intensities[:7] if i >= 0.8) >= 2:
