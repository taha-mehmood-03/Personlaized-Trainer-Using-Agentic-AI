            ...
            try:
                ...
                if voice_processed:
                    ...
                    user_details.update({
                        ...
                        "distress_index":      f"{distress_idx:.2f}/1.00 {'⚠️ HIGH' if distress_idx > 0.6 else 'moderate' if distress_idx > 0.35 else 'low'}",
                ...
                if alert_result.get("success"):
                    ...
                    print(f"[NODE: CRISIS_HANDLER] ⚠️ WhatsApp alert failed: {alert_result.get('error')}")
                ...
                try:
                    ...
                    if location_data.get('success'):
                        ...
                        if location_result.get("success"):
                            ...
                            print(f"[NODE: CRISIS_HANDLER] 🗺️ Maps: {location_result.get('maps_link')}")
                        else:
                            print(f"[NODE: CRISIS_HANDLER] ⚠️ Location alert failed: {location_result.get('error')}")
                    else:
                        print(f"[NODE: CRISIS_HANDLER] ⚠️ Could not determine location: {location_data.get('error')}")
                        
                except Exception as location_error:
                    print(f"[NODE: CRISIS_HANDLER] ⚠️ Error sending location alert: {location_error}")
                
            except Exception as alert_error:
                print(f"[NODE: CRISIS_HANDLER] ⚠️ Error sending WhatsApp alert: {alert_error}")
