...
class MentalHealthState(TypedDict):
    ...
    user_id: str
    session_id: str
    anonymous_mode: bool  # When True: skip pgvector writes and long-term memory storage
    ...
    requires_technique_series: bool        # mood LLM: symptoms span physical + cognitive domains → deliver 2 exercises
    ...
    whatsapp_alert_sent: bool   # True once a WhatsApp crisis alert has been sent this session (dedup guard)
    ...
    acoustic_distress_proxy: Optional[float]  # Real DSP distress estimate (librosa/parselmouth), cross-check for Gemini's holistic score
    has_voice: bool
    ...
    # ============================================
    # Technique outcome lifecycle tracking
    # ============================================
    technique_rejection_count: int          # increments each time a distinct technique is reported as not helping
    rejection_followup_pending: bool        # True after handle_technique_rejection; cleared next turn to trigger proactive re-offer
    last_technique_outcome: Optional[str]   # "helped" | "partial" | "not_helped"


    clinical_severity: str            # "minimal"|"mild"|"moderate"|"moderately_severe"|"severe"
    clinical_phq9_score: int          # aggregated PHQ-9 (takes max across history — never decreases)
    clinical_gad7_score: int          # aggregated GAD-7 (takes max across history — never decreases)
    clinical_raw_phq9: int            # raw per-turn PHQ-9 from LLM (can decrease as user improves)
    clinical_raw_gad7: int            # raw per-turn GAD-7 from LLM (can decrease as user improves)
    ...
    clinical_delta: Optional[float]   # v14.1: signed delta from session-start PHQ-9 (negative = improving)
    session_start_clinical_score: Optional[float]  # first PHQ-9 baseline for delta computation
    session_start_gad7_score: Optional[float]       # first GAD-7 baseline for within-session delta
    ...
    # pending_technique_context_snapshot: clinical signals at the time the pending technique was selected.
    pending_technique_context_snapshot: Optional[dict]
    # ============================================
    # SERIES COMPLEMENT TRACKING
    # ============================================
    # pending_complement_technique: second exercise queued in a series (e.g. cognitive
    # complement alongside an immediate-regulation breathing exercise). Surfaced as an
    # explicit offer after the primary technique, and promoted to recommended_technique
    # when the user accepts it or raises the signal it targets. Persists across turns.
    pending_complement_technique: Optional[dict]
    # pending_complement_signal: human-readable description of the remaining signal the
    # complement addresses (e.g. "the feeling of injustice", "the racing thoughts").
    pending_complement_signal: Optional[str]
    ...
    question_budget: int
    reflection_questions_since_resolution: int
    session_disclosure_complete: bool
...
def get_initial_state() -> MentalHealthState:
    ...
    return MentalHealthState(
        ...
        requires_technique_series=False,
        ...
        # Technique outcome lifecycle tracking
        technique_rejection_count=0,
        rejection_followup_pending=False,
        last_technique_outcome=None,
        ...
        clinical_delta=None,
        session_start_clinical_score=None,
        session_start_gad7_score=None,
        ...
        session_disclosure_complete=False,
