...
def analyze_mood(message: str) -> dict:
    ...
    try:
        if not message or not isinstance(message, str):
            print("[MOOD_TOOLS]  Invalid input - returning neutral")
            return {
                "emotion": "neutral",
                "sentiment": "neutral",
                "intensity": 0.5,
        ...
        if emoji_text_prepend and not has_real_words:
            ...
            print(f"[MOOD_TOOLS]  Emojitext: '{message}'  '{processed_message}'")
...
JSON:"""
        try:
            ...
            print(f"[MOOD_TOOLS]  LLM returned invalid result, falling back to neutral: {e}")
            raw_emotion = "neutral"
            confidence = 0.5
        # 
        ...
        go_emotions_map = {
            ...
            'anger': 'anger', 'annoyance': 'anger', 'disapproval': 'sadness', 'disgust': 'disgust',
            # Sadness cluster
        ...
        print(f"[MOOD_TOOLS]  Returning neutral fallback")
        return {
            "emotion": "neutral",
            "sentiment": "neutral",
            "intensity": 0.5,
