...
const RHYTHM: { phase: Phase; label: string; duration: number; color: string }[] = [
    { phase: 'in', label: 'Breathe In', duration: 4000, color: 'from-emerald-400 to-cyan-500' },
    { phase: 'hold', label: 'Hold', duration: 4000, color: 'from-cyan-400 to-blue-500' },
    { phase: 'out', label: 'Breathe Out', duration: 6000, color: 'from-blue-500 to-slate-700' },
]
    ...
    return (
        <div className="flex flex-col items-center gap-6 py-4">
            ...
            <div className="flex items-center gap-4 text-xs font-semibold">
                {RHYTHM.map((r, i) => (
                    <div key={r.phase} className={`flex flex-col items-center gap-1 transition-all ${running && i === phaseIdx ? 'text-cyan-700' : 'text-slate-400'}`}>
                        <div className={`w-2 h-2 rounded-full ${running && i === phaseIdx ? 'bg-cyan-500' : 'bg-slate-200'}`} />
                        <span>{r.label}</span>
